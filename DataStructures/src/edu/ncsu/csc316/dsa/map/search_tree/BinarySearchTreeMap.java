...
public class BinarySearchTreeMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
    ...
    protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
        ...
        private void relink(BinaryTreeNode<Entry<K, V>> parent, BinaryTreeNode<Entry<K, V>> child,
            ...
            if (child != null && child.getElement() != null) {
                child.setParent(parent);  // child's parent becomes parent
            }
            if (makeLeftChild) {
        ...
        public void rotate(Position<Entry<K, V>> p) {
            ...
            BinaryTreeNode<Entry<K, V>> grandparent = validate(parent(parent));
            
            if (grandparent == null) {
                setRoot(node);
                node.setParent(null);
            } else {
                relink(grandparent, node, parent == left(grandparent));
            }
            
            if (node == left(parent)) {
            	relink(parent, (LinkedBinaryTree.BinaryTreeNode<Map.Entry<K, V>>) right(node), true); 
                relink(node, parent, false);
            } else {
            	relink(parent, (LinkedBinaryTree.BinaryTreeNode<Map.Entry<K, V>>) left(node), false);
        ...
        public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
            ...
            if ((node == right(parent)) == (parent == right(parent(parent)))) {
